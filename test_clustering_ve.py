...
def make_dictionary(hashtag,df_clus):
	...
	if len(list_events_loc)!=0:
		...
		if len(time1)!=0:
			...
			dictionary_temporary[event_id]={"latitude":di[real_location][0],"longitude":di[real_location][1],"ename":event_name,"created_at":y,"loc":real_location,"text":"event{0}".format(t),"main_event":1}
	...
	for i,row in df_clus.iterrows():
			...
			if location:
				for key in di:
						...
						flag=1
				if flag==1:		
					dictionary_temporary[row["id_str"]]={"latitude":lat_long[0],"longitude":lat_long[1],"ename":event_name,"created_at":row["created_at"],"loc":location,"text":row["full_text"],"main_event":0}
